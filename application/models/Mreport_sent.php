<?php
class mreport_sent extends CI_Model {
	...
	function getData(){
        $this->db->select('p.ID, p.No, DATE_FORMAT(p.Tanggal,"%d-%m-%Y") Tanggal, u1.Nama as FinalStop, u2.Nama as Pengirim, p.Keterangan, k.Nama Kurir, p.Status');        
		...
		$this->db->where('p.Pengirim in ('.$this->session->userdata('userreport').')',NULL,FALSE);		

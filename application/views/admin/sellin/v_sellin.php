        ...
        <div class="panel panel-info">
                ...
                <div class="panel-body">
                    ...
                    <div class="table-responsive">
                        <table id="tblSellin" class="table display color-table success-table">
                            <thead>
                                <tr>                            
                                    <th></th>                            
                                    <th>Sellin No</th>
 ...
 <script type="text/javascript">
	...
	{		
 	...
 	$(document).ready(function(){	
		...
		$('#tblSellin thead tr').clone(true).appendTo( '#tblSellin thead' );
		$('#tblSellin thead tr:eq(1) th').each( function (i) {
			var title = $(this).text();
			if (title != "") 
			{
		...
		var table = $('#tblSellin').DataTable({
			"orderCellsTop": true,
        	"fixedHeader": true,
			"pageLength" : 10,
			"ajax": {
				url : "<?php echo base_url();?>admin/sellin/get_list_sellin",
				type : 'POST',
				data : function(data) {					
		...
		$('#tblSellin tbody').on('click', 'button.btnEdit', function () {
			var data = table.row( $(this).parents('tr') ).data();			
			location.href="<?php echo base_url();?>admin/sellin/edit_sellin/" + data[0];		
		});
		...
		$("#btnNew").click(function() {		
			location.href="<?php echo base_url();?>admin/sellin/edit_sellin/0";	
		});	
		
		$('#tblSellin tbody').on('click', 'button.btnDelete', function () {
			var data = table.row( $(this).parents('tr') ).data();			
			swal({
			  title: "Warning",
			  text: "Are you sure ?",
			...
			function(){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>admin/sellin/delete_sellin",
					data: {id : data[0] },
					dataType: 'json',
		...
		$('#tblSellin tbody').on('click', 'button.btnView', function () {
			var data = table.row( $(this).parents('tr') ).data();			
			location.href="<?php echo base_url();?>admin/sellin/view_sellin/" + data[0];		
		});

                        ...
                        <table id="tblPosm" class="table display color-table success-table">
                            <thead>
                                <tr>                            
                                    ...
                                    <th>Salesman</th>
                                    <th>Customer</th>
									<th>Not Visit Reason</th>
									<th>Not Buy Reason</th>
                                    <!-- <th>Total</th>-->
                                </tr>
                            </thead>   
                            <tfoot>
	...
	{		
 	...
 	$(document).ready(function(){	
		...
		var table = $('#tblPosm').DataTable({
			...
			"pageLength" : 10,
			"ajax": {
				url : "<?php echo base_url();?>admin/visit/get_list_visit",
				type : 'POST',
				data : function(data) {					
		...
		$('#tblPosm tbody').on('click', 'button.btnEdit', function () {
			...
			location.href="<?php echo base_url();?>admin/visit/edit_visit/" + data[0];		
		});
		...
		$("#btnNew").click(function() {		
			location.href="<?php echo base_url();?>admin/visit/edit_visit/0";	
		});	
		...
		$('#tblPosm tbody').on('click', 'button.btnDelete', function () {
			...
			function(){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>admin/visit/delete_visit",
					data: {id : data[0] },
					dataType: 'json',
		...
		$('#tblPosm tbody').on('click', 'button.btnView', function () {
			var data = table.row( $(this).parents('tr') ).data();			
			location.href="<?php echo base_url();?>admin/visit/view_visit/" + data[0];		
		});

                    ...
                    <div class="col-sm-12">
                        <div class="white-box p-l-20 p-r-20">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">                                    
                                    <div class="form-group">
                                        <a href="<?php echo base_url('admin/target/download_template') ?>">Download Template</a> 	     	
                                    </div>
                                </div>
                            </div>                     
					 ...
					 reader.onload = function (e) {  
						 ...
						 if (exceljson.length > 0 && cnt == 0) {  						 	 
							 ...
							 for (var i = 0; i < exceljson.length; i++) { 									   
								...
								var cellValueYear = exceljson[i]['Year'];  										 
								if (cellValueYear == null)  
									cellValueYear = "";  									
								var cellValueCycle = exceljson[i]['Cycle'];  										 
								if (cellValueCycle == null)  
									cellValueCycle = "";  
								var cellValueSalesman = exceljson[i]['Salesman'];  										 
								if (cellValueSalesman == null)  
									cellValueSalesman = "";  
								var cellValueMaterialGroup = exceljson[i]['Material Group'];  										 									if (cellValueMaterialGroup == null)  
									cellValueMaterialGroup = "";  
								var cellValueCall = exceljson[i]['Call'];  										 
								if (cellValueCall == null)  
									cellValueCall = ""; 
								var cellValueEffectiveCall = exceljson[i]['Effective Call'];  										 									if (cellValueEffectiveCall == null)  
									cellValueEffectiveCall = ""; 
								var cellValueNota = exceljson[i]['Nota'];  	
								if (cellValueNota == null)  
									cellValueNota = ""; 
								var cellValueVolume = exceljson[i]['Volume'];  	
								if (cellValueVolume == null)  
									cellValueVolume = ""; 
								
								
								TableData[i]={
									"year" : cellValueYear.trim()
									, "cycle" : cellValueCycle.trim()
									, "userid" : cellValueSalesman.trim()
									, "materialgroup" : cellValueMaterialGroup.trim()
									, "call" : cellValueCall.trim()
									, "effectivecall" : cellValueEffectiveCall.trim()
									, "nota" : cellValueNota.trim()
									, "volume" : cellValueVolume.trim()
								}    									
							 ...
							 $.ajax({
								type: "POST",
								url: "<?php echo base_url();?>admin/target/upload",

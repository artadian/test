 				...
 				<div class="row">
                    <div class="col-sm-12">
                        <div class="white-box p-l-20 p-r-20">
                            <h3 class="box-title m-b-0">Tracking Document</h3>
                            <p class="text-muted m-b-30 font-13"></p>
                            <div class="row">
                                <div class="col-md-4">                                    
                                    <div class="form-group">
                                        <input type="text" id="txtTrackDocument" class="form-control" placeholder="Enter Document No" value="<?php print $nodoc; ?>" >	
                                    </div>
                                </div>
                                <div class="col-md-2">                                    
                                    <div class="form-group">
                                        <button id="btnTrackDocument" class="btn btn-block btn-info btn-rounded"><i class="fa fa-search"></i> SEARCH</button>                  
                                   	</div>
                                </div>
                                <div class="col-md-6">                                                                        
                                </div>
 	...
 	$(document).ready(function(){			
		...
		$("#btnTrackDocument").click(function() {			
			...
			else
			{
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>admin/trackingdocument/search",
					...
					success: function(jsonData){												
						if (jsonData['status'] == 'ok')
						{
							...
							str = str + '		</div>';
							str = str + '		<div class="col-md-4 col-xs-6 b-r"> <strong>From</strong>';
							str = str + '			<br>';
							str = str + '			<p class="text-muted">'+ jsonData['header'].Pengirim +'</p>';
							str = str + '		</div>';
							str = str + '		<div class="col-md-4 col-xs-6"> <strong>Duration</strong>';
							str = str + '			<br>';
							...
							{								
								if (jsonData['data'][i].Status == 'COMPLETE')
								{
									...
									str = str + '		  <div class="tracking-content">[COMPLETE] '+ jsonData['data'][i].Tujuan +'<span>['+ jsonData['data'][i].PaketNo +'] Sent by '+ jsonData['data'][i].Kurir +', Received by '+ jsonData['data'][i].Penerima +'</span></div>';
									str = str + '	   </div>';
								}
								else if (jsonData['data'][i].Status == 'RECEIVED')
								{
									...
									str = str + '		  <div class="tracking-date">'+ jsonData['data'][i].TglKirim +'<span>'+ jsonData['data'][i].TglTerima +'</span></div>';
									str = str + '		  <div class="tracking-content">[RECEIVED] '+ jsonData['data'][i].Tujuan +'<span>['+ jsonData['data'][i].PaketNo +'] Sent by '+ jsonData['data'][i].Kurir +', Received by '+ jsonData['data'][i].Penerima +'</span></div>';
									str = str + '	   </div>';
								}
								else if (jsonData['data'][i].Status == 'PROGRESS')
								{
									...
									str = str + '		  <div class="tracking-date">'+ jsonData['data'][i].TglKirim +'<span>'+ jsonData['data'][i].TglTerima +'</span></div>';
									str = str + '		  <div class="tracking-content">[PROGRESS] '+ jsonData['data'][i].Tujuan +'<span>['+ jsonData['data'][i].PaketNo +'] Sent by '+ jsonData['data'][i].Kurir +', Received by '+ jsonData['data'][i].Penerima +'</span></div>';
									str = str + '	   </div>';
								}
								else if (jsonData['data'][i].Status == 'STOP')
								{
									...
									str = str + '		  <div class="tracking-date">'+ jsonData['data'][i].TglKirim +'<span>'+ jsonData['data'][i].TglTerima +'</span></div>';
									str = str + '		  <div class="tracking-content">[STOP] '+ jsonData['data'][i].Tujuan +'<span>['+ jsonData['data'][i].PaketNo +'] Sent by '+ jsonData['data'][i].Kurir +', Received by '+ jsonData['data'][i].Penerima +'</span></div>';
									str = str + '	   </div>';
								}
							}						
						...
						{
							swal('Warning','Document not found','warning');				
							return false; 
						}												
					},
		...
		<?php if ($nodoc != '') { ?>
		$("#btnTrackDocument").trigger("click");
		<?php } ?>

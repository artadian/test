        ...
        <div class="panel panel-info">
                ...
                <div class="panel-body">
                    ...
                    <div class="table-responsive">
                        <table id="tblStock" class="table display color-table success-table">
                            <thead>
                                <tr>                            
                                    <th></th>          
                                    <th>Stock Date</th>
...
<script type="text/javascript">
	...
	{		
 	...
 	$(document).ready(function(){	
		...
		$('#tblStock thead tr').clone(true).appendTo( '#tblStock thead' );
		$('#tblStock thead tr:eq(1) th').each( function (i) {
			var title = $(this).text();
			if (title != "") 
			{
		...
		var table = $('#tblStock').DataTable({
			...
			"pageLength" : 10,
			"ajax": {
				url : "<?php echo base_url();?>admin/stock/get_list_stock",
				type : 'POST',
				data : function(data) {					
		...
		$('#tblStock tbody').on('click', 'button.btnEdit', function () {
			var data = table.row( $(this).parents('tr') ).data();			
			location.href="<?php echo base_url();?>admin/stock/edit_stock/" + data[0];		
		});
		...
		$("#btnNew").click(function() {		
			location.href="<?php echo base_url();?>admin/stock/edit_stock/0";	
		});	
		
		$('#tblStock tbody').on('click', 'button.btnDelete', function () {
			var data = table.row( $(this).parents('tr') ).data();			
			swal({
			  title: "Warning",
			  text: "Are you sure ?",
			...
			function(){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>admin/stock/delete_stock",
					data: {id : data[0] },
					dataType: 'json',
		...
		$('#tblStock tbody').on('click', 'button.btnView', function () {
			var data = table.row( $(this).parents('tr') ).data();			
			location.href="<?php echo base_url();?>admin/stock/view_stock/" + data[0];		
		});

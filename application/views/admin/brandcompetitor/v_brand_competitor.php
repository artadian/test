        ...
        <div class="panel panel-info">
                ...
                <div class="panel-body">
                	<div class="row">            	
                        <div class="col-md-6">                                    
                            <div class="form-group">
                                ...
                                <select id="cmbCompetitor" class="form-control select2">
                    ...
                    <div class="table-responsive">
                        <table id="tblBrandCompetitor" class="table display color-table success-table">
                            <thead>
                                <tr>                            
                                    <th></th>           
                                    <th>Competitor</th>                                   
...
<!-- /.row -->
 ...
 <script type="text/javascript">
 	...
 	$(document).ready(function(){
 		$("#cmbCompetitor").select2();

		$('#tblBrandCompetitor thead tr').clone(true).appendTo( '#tblBrandCompetitor thead' );
		$('#tblBrandCompetitor thead tr:eq(1) th').each( function (i) {
			var title = $(this).text();
			if (title != "") 
			{
		...
		var table = $('#tblBrandCompetitor').DataTable({
			"orderCellsTop": true,
        	"fixedHeader": true,
			"pageLength" : 10,
			"ajax": {
				url : "<?php echo base_url();?>admin/brandcompetitor/get_list_brand_competitor",
				type : 'POST',
				data : function(data) {	
					data.competitor = $("#cmbCompetitor").val();
				}				
			},
			'columnDefs': [
			  {
		...
		$('#tblBrandCompetitor tbody').on('click', 'button.btnEdit', function () {
			var data = table.row( $(this).parents('tr') ).data();			
			location.href="<?php echo base_url();?>admin/brandcompetitor/edit_brand_competitor/" + data[0];		
		});
		...
		$("#btnNew").click(function() {		
			location.href="<?php echo base_url();?>admin/brandcompetitor/edit_brand_competitor/0";	
		});	
		
		$('#tblBrandCompetitor tbody').on('click', 'button.btnDelete', function () {
			var data = table.row( $(this).parents('tr') ).data();			
			swal({
			  title: "Warning",
			  text: "Are you sure ?",
			...
			function(){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>admin/brandcompetitor/delete_brand_competitor",
					data: {id : data[0] },
					dataType: 'json',
		...
		$('#tblBrandCompetitor tbody').on('click', 'button.btnView', function () {
			var data = table.row( $(this).parents('tr') ).data();			
			location.href="<?php echo base_url();?>admin/brandcompetitor/view_brand_competitor/" + data[0];		
		});

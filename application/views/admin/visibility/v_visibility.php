        ...
        <div class="panel panel-info">
                ...
                <div class="panel-body">
                    ...
                    <div class="table-responsive">
                        <table id="tblVisibility" class="table display color-table success-table">
                            <thead>
                                <tr>                            
                                    <th></th>           
 ...
 <script type="text/javascript">
	...
	{		
 	...
 	$(document).ready(function(){	
		...
		$('#tblVisibility thead tr').clone(true).appendTo( '#tblVisibility thead' );
		$('#tblVisibility thead tr:eq(1) th').each( function (i) {
			var title = $(this).text();
			if (title != "") 
			{
		...
		var table = $('#tblVisibility').DataTable({
			"orderCellsTop": true,
        	"fixedHeader": true,
			"pageLength" : 10,
			"ajax": {
				url : "<?php echo base_url();?>admin/visibility/get_list_visibility",
				type : 'POST',
				data : function(data) {					
		...
		$('#tblVisibility tbody').on('click', 'button.btnEdit', function () {
			var data = table.row( $(this).parents('tr') ).data();			
			location.href="<?php echo base_url();?>admin/visibility/edit_visibility/" + data[0];		
		});
		...
		$("#btnNew").click(function() {		
			location.href="<?php echo base_url();?>admin/visibility/edit_visibility/0";	
		});	
		
		$('#tblVisibility tbody').on('click', 'button.btnDelete', function () {
			var data = table.row( $(this).parents('tr') ).data();			
			swal({
			  title: "Warning",
			  text: "Are you sure ?",
			...
			function(){
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url();?>admin/visibility/delete_visibility",
					data: {id : data[0] },
					dataType: 'json',
		...
		$('#tblVisibility tbody').on('click', 'button.btnView', function () {
			var data = table.row( $(this).parents('tr') ).data();			
			location.href="<?php echo base_url();?>admin/visibility/view_visibility/" + data[0];		
		});
